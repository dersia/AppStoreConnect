...
namespace AppStoreConnect.Endpoints.Certificates
{
    ...
    {
        public async Task<ApplicationResponse> CreateCertificate(CertificateCreateRequest payload)
        {
            ...
            {
                { StatusCode: 201 } br => JsonSerializer.Deserialize<CertificateResponse>(await br.Response.Content.ReadAsStringAsync()),
                { StatusCode: 400 } br => JsonSerializer.Deserialize<ErrorResponse>(await br.Response.Content.ReadAsStringAsync()),
                { StatusCode: 401 } br => JsonSerializer.Deserialize<ErrorResponse>(await br.Response.Content.ReadAsStringAsync()),
