...
namespace AppStoreConnect.Endpoints.UserInvitations
{
    public partial class UserInvitations : EndpointBase, IUserInvitations
    {
        ...
        {
            var endpointUrl = $"{EndpointConstants.AppStoreConnectBaseUrl}/{EndpointConstants.Enpoints.UserInvitations}/{id}/{EndpointConstants.Enpoints.Relationships}/{EndpointConstants.Enpoints.VisibleApps}";
            if (filters is { })
            {
            ...
            {
                { StatusCode: 200 } br => JsonSerializer.Deserialize<UserInvitationVisibleAppsLinkagesResponse>(await br.Response.Content.ReadAsStringAsync()),
